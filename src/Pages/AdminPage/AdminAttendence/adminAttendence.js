...
    // 🟢 Online cards (Logic to show actively clocked-in, 'Active' status employees today)
...
            !row.checkOut &&
            row.status === 'Active' // Explicitly check for Active status
        );
    }, [attendanceData]);
...
                                    <td>
                                      <button className="btn-table-edit" onClick={() => openEditModal(row)}>Edit</button>
                                      <button className="btn-table-delete" onClick={() => handleDeleteAttendance(row)}>Delete</button>
                                    </td>
                                </tr>
                            ))}
                            {filteredData.length === 0 && (
                                <tr><td colSpan="10">No records found matching filters.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
...
                                <strong>{emp.employee || emp.employeeName || 'Unknown Employee'}</strong> 
                                <p>ID: {emp.empId}</p>
                                <p>Check-In: {emp.checkIn || 'N/A'}</p>
                            </div>
...
                {confirmDelete.show && (
                  <ConfirmToast
                    message={`Delete attendance for ${confirmDelete.record?.employee || confirmDelete.record?.empId || ''} on ${confirmDelete.record?.date || ''}?`}
